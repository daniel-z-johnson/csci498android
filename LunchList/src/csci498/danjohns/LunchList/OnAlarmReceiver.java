...
public class OnAlarmReceiver extends BroadcastReceiver {
	private static final int NOTIFY_ME_ID = 16*9*5*7*11*13*17*19; //Project Euler solution, why not?
	
	@Override
	public void onReceive(Context ctxt, Intent intent) {
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(ctxt);
		boolean useNotifications = prefs.getBoolean("use_notification", true);
		
		if (useNotifications) {
			NotificationManager mgr = (NotificationManager)ctxt.getSystemService(Context.NOTIFICATION_SERVICE);
			
			Notification note = new Notification(R.drawable.stat_notify_chat, "It's time for linch!", System.currentTimeMillis());
